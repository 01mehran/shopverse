...
export default function CartItem({ id }: cartItem) {
  ...
  return (
    <section className="relative flex w-full gap-2 md:gap-4">
      {/* Product Image */}
      <img
        src={data?.data.image}
        alt="clothes"
      ...
      <div className="flex w-full flex-col justify-between">
        <div className="flex flex-col gap-px">
          <p className="text-base font-bold sm:text-xl">{data?.data.name}</p>
        ...
        <div className="flex w-full justify-between">
          <p className="text-[24px] font-bold">${data?.data.price}</p>
